...
•	Remove participants with values considered outliers on ‘FACEenmesh’
...
```{r}
data5 = data4 %>% 
  mutate(siblings_check = case_when(Siblings > 0 ~ "YES",
                              TRUE ~ 'NO')) %>% 
  select(siblings_check,everything())

shapiro.test(data5$FACEenmesh[data5$siblings_check=='YES'])

shapiro.test(data5$FACEenmesh[data5$siblings_check=='NO'])

shapiro.test(data5$FACEenmesh[data5$siblings_check=='NO'] -data5$FACEenmesh[data5$siblings_check=='YES'])

hist(data5$FACEenmesh[data5$siblings_check=='YES'])

hist(data5$FACEenmesh[data5$siblings_check=='NO'])

wilcox.test(data5$FACEenmesh~data5$siblings_check)
```
